<script lang="ts">
	...
	const size = 8;
	const BORDER_COLOR = "#c595ff";
	let cubeSize = $state(20);
	let viewGrid = $state(true);
	let editing = $state(false);
	let selectedLayer = $state(0);
	let openNav = $state(true);
	...
	function loadCube() {
		for (let row = 0; row < 8; row++) {
			const rowCube = getCookie(`cube-${row}`);
			console.log(rowCube);
	...
	function setPersective(x: number, y: number, t: number, l: number) {
	...
	function setViewGrid(b: boolean) {
		viewGrid = b;
	}

	function cubeClick(e: MouseEvent, x: number, y: number, z: number) {
	...
	function closeEidting() {
		editing = false;
	}
	function setColor(color: RGB | null) {
...
{#if selectedLayer == 0}
	<div class=" absolute bottom-5 left-5">
		<ViewPanel {setPersective} {setViewGrid} {viewGrid} />
	</div>
{/if}
